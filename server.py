...
def _analyze(symbol: str, ticker: str, df, config: dict | None = None, benchmark_close=None):
    ...
    support1 = (
        supports[0]["price"]
        if supports
        else _finite(low.shift(1).rolling(60).min().iloc[-1])
    )
    ...
    breakout_score += (
        ...
        else max(0, 18 - min(18, max(0, resistance_dist) * 4))
    )
    ...
    ema_spread = _finite(
        (
            ...
            - min(ema20.iloc[-1], ema50.iloc[-1], ema200.iloc[-1])
        )
        / last_close
        * 100
    )
    ...
    setup = (
        "Breakout"
        if donchian_breakout and breakout_score >= 65
        else (
            "Sıkışma"
            if squeeze_score >= 70
            else (
                "Güçlü Trend"
                if ema_trend and supertrend_buy and macd_bullish
                else (
                    "Pozitif Uyumsuzluk"
                    if positive_divergence
                    else ("İzleme" if score >= 50 else "Zayıf")
                )
            )
        )
    )
...
"changeYearly":round(change_yearly,2),
        ...
        "aiBreakoutProbability": ai["probability"],
        "aiRiskScore": ai["risk"],
        ...
        "upDownVolumeRatio": round(up_down_volume_ratio, 2),
        "priceVolumeCorrelation": round(pv_confirmation, 3),
        "setup": setup,
        "passed": passed,
        ...
        "resistance2": round(_finite(resistance2), 2),
        "supportDistancePct": round(support_dist, 2),
        "resistanceDistancePct": round(resistance_dist, 2),
        "supportTouches": supports[0]["touches"] if supports else 0,
        "resistanceTouches": resistances[0]["touches"] if resistances else 0,
        "levelTolerance": round(_finite(level_tolerance), 2),
        "ema20": round(_finite(ema20.iloc[-1]), 2),
        "ema50": round(_finite(ema50.iloc[-1]), 2),
        "ema200": round(_finite(ema200.iloc[-1]), 2),
        "macd": round(_finite(macd_line.iloc[-1]), 3),
        "macdSignal": round(_finite(macd_signal.iloc[-1]), 3),
        "supertrend": round(_finite(st_line.iloc[-1]), 2),
        "market": "BIST",
        "dataDate": (
            clean.index[-1].strftime("%Y-%m-%d")
            if hasattr(clean.index[-1], "strftime")
